...
wire               [63:0] CORE_ID;
wire                      is_reduntant_core;
wire                      dcls_enable_split;
reg                       found_primary_core;

assign dcls_enable_split = 1'b0;


initial begin
    int i;
    string hierarchy_str;
    string primary_core_str;

    found_primary_core = 1'b0;
    $sformat(hierarchy_str, "%m");
    $sformat(primary_core_str, "u_ax45mpv_core_pair.u_core0");

    for (i = 0; i < hierarchy_str.len; i++) begin
        if (hierarchy_str.substr(i, i + primary_core_str.len() - 32'd1) == primary_core_str) begin
            found_primary_core = 1'b1;
        end
    end
    found_primary_core = 1'b1;
...
assign is_reduntant_core = (~found_primary_core) & (~dcls_enable_split);
assign CORE_ID           = `NDS_CORE.hart_id;
...
always @(posedge core_clk) begin
    if (i0_inst_retire & (~is_reduntant_core)) begin
        $display("%0t:ipipe:%0d:@%h=%h",$time, CORE_ID, wb_i0_pc, wb_i0_instr);
    end
...
always @(posedge core_clk or negedge core_reset_n) begin
    if (~core_reset_n) begin
        ...
        if (idle_counter >= `NDS_IPIPE_TIMEOUT) begin
            $display("%0t:ipipe:%0d:ERROR:%0d cpu cycles passed without instruction retirement", $time, CORE_ID, idle_counter);
            wrapup();
        end
    end
end
...
always @(`NDS_SIM_CONTROL.event_finish) begin
    finish_counter ++;
    if ((finish_counter == `NDS_ACTIVE_NHART) & (~is_reduntant_core)) begin
        $display("%0t:simple_ipipe:%0d:---- simulated in %0d cpu cycles", $time, CORE_ID, instn_simple_pipe_mon.cpu_cycles);
    end
end
