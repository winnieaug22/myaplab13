...
`else
	...
	//`define AE350_OSCL_PERIOD 3051.758
`endif
...
initial begin
	...
	fd = $fopen("output_custom.hex", "w"); // Open file in write mode
        
        if (fd) begin
            for (i = 0; i < IMAGE_SIZE; i = i + 1) begin
                
                // Print the address marker (@) at the beginning of a new line
                if (i % 4 == 0) begin
                    $fwrite(fd, "@%08x ", i);
                end
                
                // Print the memory data
                $fwrite(fd, "%08x ", data[i]);
                
                // Print a newline character after every 4 words
                if (i % 4 == 3) begin
                    $fwrite(fd, "\n");
                end
            end
            
            // Catch-all: Print a final newline if the depth is not a multiple of 4
            if (IMAGE_SIZE % 4 != 0) begin
                $fwrite(fd, "\n");
            end
            
            $fclose(fd); // Always close the file to ensure data is saved
            $display("Success: Dumped memory to output_custom.hex.");
        end
	//#CYC]
	// synthesis translate_on
...
`ifdef BYPASS_FLASH
initial begin
	force system.ae350_chip.ae350_vcpu_cluster_subsystem.hart0_reset_vector[31:0] = 32'h00000080;
	force system.ae350_chip.ae350_vcpu_cluster_subsystem.hart1_reset_vector[31:0] = 32'h00000080;
	force system.ae350_chip.ae350_vcpu_cluster_subsystem.hart2_reset_vector[31:0] = 32'h00000080;
	force system.ae350_chip.ae350_vcpu_cluster_subsystem.hart3_reset_vector[31:0] = 32'h00000080;
	force system.ae350_chip.ae350_vcpu_cluster_subsystem.hart4_reset_vector[31:0] = 32'h00000080;
	force system.ae350_chip.ae350_vcpu_cluster_subsystem.hart5_reset_vector[31:0] = 32'h00000080;
	force system.ae350_chip.ae350_vcpu_cluster_subsystem.hart6_reset_vector[31:0] = 32'h00000080;
	force system.ae350_chip.ae350_vcpu_cluster_subsystem.hart7_reset_vector[31:0] = 32'h00000080;
end
`endif
